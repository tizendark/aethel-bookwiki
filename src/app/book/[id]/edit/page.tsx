...
import { authService, bookService, editService, mediaService } from "@/services";
...
export default function EditBookPage() {
  ...
  const { user, isModerator, isLoading: authChecking } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (coverFile) {
        const uploadedUrl = await mediaService.uploadImage(coverFile);
      ...
      if (isModerator) {
        ...
        // Guardar en la colección `edits` (Proposición) via editService
        await editService.proposeEdit({
          bookId: id,
          proposedTitle: title,
          ...
          authorId: user.uid,
          author: user.displayName || "Usuario Anónimo"
        });
      }
