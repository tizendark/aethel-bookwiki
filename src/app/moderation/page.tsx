...
import { authService, bookService, editService } from "@/services";
import { Book, EditProposal } from "@/types";
...
interface PendingBook extends Book {
  authorName?: string;
}

export default function ModerationPage() {
  const { isModerator, isLoading: isCheckingAuth } = useAuth();
  ...
  const [previewItem, setPreviewItem] = useState<PendingBook | EditProposal | null>(null);
  const [previewPage, setPreviewPage] = useState(0);

  const openPreview = (item: PendingBook | EditProposal) => {
    setPreviewItem(item);
    setPreviewPage(0);
  };
  ...
  const fetchPendingBooks = async () => {
    try {
      const loadedBooks = await bookService.getPendingBooks();
      setBooks(loadedBooks as PendingBook[]);
    } catch (err: any) {
  ...
  const fetchPendingEdits = async () => {
    try {
      const loadedEdits = await editService.getPendingEdits();
  ...
  const handleApproveBook = async (book: PendingBook) => {
    try {
      ...
      await bookService.approveBook(book.id);
  ...
  const handleRejectBook = async (bookId: string) => {
    ...
    try {
      ...
      await bookService.rejectBook(bookId);
  ...
  const handleApproveEdit = async (edit: EditProposal) => {
    try {
      setProcessingId(edit.id);
      await editService.approveEdit(edit.id);
  ...
  const handleRejectEdit = async (editId: string) => {
    ...
    try {
      ...
      await editService.rejectEdit(editId);
      ...
      <AnimatePresence>
        {previewItem && (
          <PreviewModal 
            ...
            onApprove={() => {
              if ('proposedTitle' in previewItem) handleApproveEdit(previewItem as EditProposal);
              else handleApproveBook(previewItem as PendingBook);
              closePreview();
            }}
            onReject={() => {
              if ('proposedTitle' in previewItem) handleRejectEdit((previewItem as EditProposal).id);
...
function PreviewModal({ item, currentPage, setPage, onClose, onApprove, onReject, processingId }: any) {
  ...
  const author = isEdit ? item.author : (item.author || 'Usuario Anónimo');
