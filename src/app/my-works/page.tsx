...
import { Book, AuthUser, NewBookInput, EditProposalInput } from "@/types";
...
export default function MyWorksPage() {
  const { user, isLoading: authChecking } = useAuth();
  
  const [publishedBooks, setPublishedBooks] = useState<Book[]>([]);
  const [pendingBooks, setPendingBooks] = useState<Book[]>([]);
  const [localDraft, setLocalDraft] = useState<NewBookInput | null>(null);
  const [localEditDrafts, setLocalEditDrafts] = useState<(EditProposalInput & { bookId: string })[]>([]);
  ...
  const fetchUserWorks = async (userId: string) => {
    setIsLoading(true);
    try {
      // Fetch Published Books using service
      const pubs = await bookService.getBooksByAuthor(userId);
      setPublishedBooks(pubs);

      // Fetch Pending Books using service
      const pens = await bookService.getPendingBooksByAuthor(userId);
